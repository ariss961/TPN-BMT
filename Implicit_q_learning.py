...
# In the GLOBAL PARAMETERS AND CONFIGURATION section, with your other imports
...
def calculate_hybrid_reward_v2(
    ...
    Simplified to remove sigma_scale dependency.
    ...
    for val, col in zip(predicted_measurement_vector, measurement_cols):
        ...
        if lo <= val <= hi:
            ...
            # Maximum bonus when at target, decreasing linearly to edges
...
def filter_measurements_by_tpn_windows(patient_id):
    ...
    """Keep only measurements within ±4h of any TPN dose for this patient"""

    patient_tpn = df_main[df_main['person_id'] == patient_id]['order_inst'].values

    patient_meas = df_meas[df_meas['person_id'] == patient_id].copy()

   

    if len(patient_tpn) == 0 or len(patient_meas) == 0:

        return pd.DataFrame()

   

    tpn_times = pd.to_datetime(patient_tpn)

    valid_measurements = []

   

    for idx, meas_row in patient_meas.iterrows():

        meas_time = pd.to_datetime(meas_row[MEAS_TIMESTAMP_COL])

        within_window = False

        for tpn_time in tpn_times:

            time_diff_hours = abs((meas_time - tpn_time).total_seconds() / 3600)

            if time_diff_hours <= TPN_WINDOW_HOURS:
        ...
        if within_window:
            ...
            valid_measurements.append(meas_row)

   

    return pd.DataFrame(valid_measurements) if valid_measurements else pd.DataFrame()
...
for patient_id in tqdm(common_patients, desc="Filtering measurements"):
    ...
    if len(filtered_patient_meas) > 0:
...
df_aligned['tpn_interval_marker_time'] = df_aligned.loc[df_aligned['is_tpn_event'] == 1,
...
def create_offline_dataset_for_iql(
    ...
    for pid in tqdm(patient_ids, desc="Building (s,a,s',r) tuples"):
        ...
        for t_idx, (tpn_time, tpn_row) in enumerate(tpn_df.iterrows()):
            ...
            # ➊ find the lab just before and just after this TPN

            prev_lab = labs_df[:tpn_time].iloc[-1:]      # t−

            # Corrected logic to get the first measurement strictly after time t

            next_lab = labs_df[tpn_time:].iloc[0:1] if not labs_df[tpn_time:].empty else pd.DataFrame()
...
# === ADD THIS CODE TO THE END OF PART 3 IN YOUR ORIGINAL TRAINING SCRIPT ===

# =========================================================================



# After creating the train/val/test datasets for IQL...



# Create a dedicated directory for the data artifacts within the run log directory

# NOTE: This assumes IQL_LOG_DIR is defined before this point.

# You might need to move the IQL_LOG_DIR definition to be earlier in the script.
...
# Using pickle to preserve data types like datetimes
...
# Get action bounds from training data
...
# Create action scaler that maps to [-1, 1]
...
# <<< MODIFICATION START: Scale Validation and Test Actions >>>
...
# Compute mean and std for observations from the training set ONLY
...
train_dataset = MDPDataset(
    ...
    actions=train_acts_scaled,  # Use scaled actions
...
# <<< MODIFICATION START: Create Validation MDPDataset >>>

val_dataset = MDPDataset(

    observations=val_obs,

    actions=val_acts_scaled, # Use scaled actions
...
print(f"Training dataset: {num_episodes} episodes / "
...
# 2. Configure the d3rlpy logger to use the PARENT directory as its root.
...
try:
    ...
    # The actual path might have a timestamp suffix added by d3rlpy.

    # We search for it to ensure we use the correct path for evaluation.



    # Use the original IQL_RUN_NAME as the search prefix
    ...
    if not possible_dirs:
        ...
        # Fallback to the original path, though it will likely fail later

        actual_log_dir = IQL_LOG_DIR

    else:

        # In case of multiple matches, sort by modification time and pick the latest one
    ...
    # IMPORTANT: Overwrite the IQL_LOG_DIR variable to point to the correct path

    # All subsequent code will now use this corrected path
    ...
    # 4. Save the final model and scaler to your guaranteed directory.
...
# Use the final model path from the training step
...
try:
    ...
    # Fallback to the latest model if final doesn't exist
...
def evaluate_offline_policy(
    ...
    dataset_acts_scaled: np.ndarray, # Takes scaled actions
...
fig, axes = plt.subplots(5, 5, figsize=(20, 20)) # Adjusted for 25 actions
...
try:
    ...
    log_dir = IQL_LOG_DIR # The path variable is now correctly pointing to the single log directory
    ...
    if not available_metrics:
        ...
        for metric_name, metric_label in available_metrics.items():
            ...
            try:
                ...
                # Read the CSV without expecting a header and manually assign column names.

                # The typical d3rlpy format is epoch, step, value.
                ...
                # Plotting now works because we have defined the 'step' and 'value' columns
...
# In PART 5, after the "Test Set Results" are printed.
...
# Use the unscaled actions from the test set evaluation results
...
# For defining the support, it's best to use the full training dataset

# as it represents all the "known" behavior.
...
for j, dose_name in enumerate(TPN_DOSE_COLS):
    ...
    # Get actions for the current dimension (j)
    ...
    # Define common bin edges based on the combined range of both distributions
    ...
    # Compute density histograms, adding a tiny epsilon to avoid log(0)
    ...
    # Calculate KL-divergence D_KL(Policy || Behavior)
...
# Define the "support" (min/max) from the entire training dataset
...
# Check if the predicted actions on the test set are within the training support

in_bounds = (predicted_actions >= min_support[None, :]) & (predicted_actions <= max_support[None, :])



# Calculate overall coverage: % of samples where *all* action dimensions are in-bounds
...
# Append these new metrics to the results_summary dictionary

results_summary['test_set_metrics']['kl_divergences'] = kl_divergences

results_summary['test_set_metrics']['overall_coverage'] = full_coverage

results_summary['test_set_metrics']['coverage_per_dim'] = coverage_per_dim.tolist()



# Re-save the JSON file with the new data
...
try:
    ...
    fig, ax = plt.subplots(figsize=(12, 10)) # Adjusted size for better readability

   

    # Create a horizontal bar plot for easier reading of labels
